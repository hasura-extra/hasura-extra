...
(new class() {
    ...
    private function getManager(): ManagerInterface
    {
        ...
        $yamlOperator = new YamlOperator(new Filesystem());
    ...
    private function getConfig(): array
    {
        ...
        if (null === $baseUri) {
            throw new \RuntimeException(
                sprintf('You should be config Hasura base uri via `%s` config file or `HASURA_BASE_URI` env.', $file)
            );
        }
        ...
        if (null === $metadataPath) {
            throw new \RuntimeException(
                sprintf(
                    'You should be config Hasura metadata path where to store metadata files via `%s` config file or `HASURA_METADATA_PATH` env.',
                    $file
                )
            );
        }
